...
class AnalysisState(BaseModel):
    ...
    current_step: Literal["Y", "W", "T"] = Field(default="Y", description="現在の分析ステップ")
...
class YWTAnalyzer:
    ...
    def analyze_y(self, topic: str, persona: Persona) -> YAnalysis:
        ...
        print_progress(f"{persona.name}のY分析が完了しました → {y_done.split('.')[0]}...")
        return analysis
    
    def analyze_w(self, topic: str, persona: Persona, y_analysis: YAnalysis) -> WAnalysis:
        ...
        print_progress(f"{persona.name}のW分析が完了しました → {w_learned.split('.')[0]}...")
        return analysis
    ...
    def analyze_t(
        ...
        topic: str,
        persona: Persona,
        y_analysis: YAnalysis,
        w_analysis: WAnalysis
    ) -> TAnalysis:
        ...
        print_progress(f"{persona.name}のT分析が完了しました → {t_next.split('.')[0]}...")
        return analysis
...
class StepAnalyzer:
    ...
    def analyze_y_step(self, topic: str, analyses: list[tuple[Persona, YAnalysis]]) -> StepSummary:
...
class MultiPersonaYWTAgent:
    ...
    def _create_graph(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {
                True: "analyze_current_step",
                False: "summarize_current_step"
            }
        )
    ...
    def _analyze_current_step(self, state: AnalysisState) -> dict[str, Any]:
        ...
        current_analysis = state.analyses[current_persona.name]
        ...
        if state.current_step == "Y":
            ...
            w_analysis = self.ywt_analyzer.analyze_w(state.topic, current_persona, y_analysis)
        ...
        return {
            "analyses": {current_persona.name: current_analysis},
    ...
    def _check_persona_completion(self, state: AnalysisState) -> dict[str, Any]:
        ...
        if is_complete:
            print_progress(f"全ペルソナの{state.current_step}分析が完了しました", is_state=True)
    ...
    def _summarize_current_step(self, state: AnalysisState) -> dict[str, Any]:
        ...
        if state.current_step == "Y":
            ...
            w_analyses = [
                ...
                for persona in state.personas
                if (
                    state.analyses[persona.name].y_analysis is not None and
                    state.analyses[persona.name].w_analysis is not None
                )
            ]
            ...
            t_analyses = [
                ...
                if (
                    state.analyses[persona.name].y_analysis is not None and
                    state.analyses[persona.name].w_analysis is not None and
                    state.analyses[persona.name].t_analysis is not None
                )
            ]
    ...
    def run(self, topic: str) -> tuple[dict[str, PersonaAnalysis], StepSummary, StepSummary, StepSummary]:
        print_section("分析開始")
